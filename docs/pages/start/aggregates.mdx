...
```
If the stream name isn't provided in the decorator, the name of the class will automatically be used (lowercase).
...
**3. Use Events to Communicate Changes**
...
**4. Handle Events to Update State**

Each event should have a counterpart that mutates the aggregate's state based on the event that was applied to aggregate (methods prefixed with `on`). This pattern decouples the business logic from how the state is updated.
...
**5. Keep aggregates focused on domain logic**
...
**6. Event handling should be idempotent**
