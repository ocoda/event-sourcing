...
1. **disable** the creation of a **default pool** in the store configuration
...
- **provide a pool** when interacting with the **event-store** or **snapshot-handler**

  ```typescript
    await this.eventStore.appendEvents(stream, account.version, events, 'tenant-1');
    await this.accountSnapshotHandler.load(accountId, 'tenant-1');
  ```