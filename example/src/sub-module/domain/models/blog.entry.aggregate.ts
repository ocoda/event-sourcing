...
import { BlogEntryCreatedEvent, BlogEntryDeletedEvent } from '../events';
...
export class BlogEntry extends AggregateRoot {
	...
	create(author: BlogEntryAuthorId, title: string, content: string, createdOn?: Date): BlogEntryId {
		// validate the stuff
		if (!author || !author.value) throw new Error('Author is required');
		...
		this.applyEvent<BlogEntryCreatedEvent>(
			new BlogEntryCreatedEvent(id.value, title, content, author.value, createdOn ?? new Date()),
		);
	...
	delete() {
		this.applyEvent(new BlogEntryDeletedEvent(this.id.value));
	}
