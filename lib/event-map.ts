...
export class EventMap {
	...
	public register<E extends Type<IEvent>>(cls: E, serializer?: IEventSerializer): void {
	...
	public createEnvelopes(aggregateId: Id, aggregateVersion: number, events: IEvent[]): EventEnvelope[] {
		let sequence = aggregateVersion - events.length + 1;
		return events.map((event) => {
			const { name, serializer } = this.get(event.constructor as Type<IEvent>);
			const envelope = EventEnvelope.new(aggregateId, sequence, name, serializer?.serialize(event) || event);
	...
	public getSerializer<E extends Type<IEvent>>(event: string | E): IEventSerializer<E> {
		const helper = this.get(event);
		if (!helper) {
			throw new UnregisteredEventException(typeof event === 'string' ? event : event.name);
		}
