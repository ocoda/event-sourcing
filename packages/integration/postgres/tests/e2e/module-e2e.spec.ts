...
describe('EventSourcingModule - e2e', () => {
	...
	let eventStoreClient: PoolClient;
	let snapshotStoreClient: PoolClient;
	...
	beforeAll(async () => {
		...
		const eventStore = app.get<PostgresEventStore>(EventStore);
		const snapshotStore = app.get<PostgresSnapshotStore>(SnapshotStore);
