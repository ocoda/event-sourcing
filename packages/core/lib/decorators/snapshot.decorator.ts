 ...
 * @description The decorated class must extend the `SnapshotRepository` or implement the `ISnapshotRepository` interface.
 ...
 * @param { Omit<SnapshotRepositoryMetadata<A>, 'aggregate'>} options The metadata for the snapshot handler.
 * @returns {ClassDecorator}
 * @example `@Snapshot(Account, { name: 'account', interval: 5 })`
 */
export const Snapshot = <A extends AggregateRoot = AggregateRoot>(
	aggregate: Type<A>,
	options?: Omit<SnapshotRepositoryMetadata<A>, 'aggregate'>,
): ClassDecorator => {
